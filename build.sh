...
if [ "$1" = "--universal" ]; then
    ...
    swiftc -target arm64-apple-macosx13.0 Sources/SSHClientsWidget/*.swift -O -o "build/${APP_NAME}-arm64" $FRAMEWORKS
    echo "Compiling (x86_64)..."
    swiftc -target x86_64-apple-macosx13.0 Sources/SSHClientsWidget/*.swift -O -o "build/${APP_NAME}-x86_64" $FRAMEWORKS
    ...
    swiftc Sources/SSHClientsWidget/*.swift -O -o "$APP/Contents/MacOS/$APP_NAME" $FRAMEWORKS
fi
